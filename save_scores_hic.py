...
class SaveScores:
    ...
    def save_triggers_launches_obs_cancellations(self, param_df):
        ...
        #make an array that defines where flux above 20 percent of peak was observed by HiC
        flareID_observations = np.array(param_df['Flare_ID'][(param_df['Cancelled?']==False) & (param_df[f'HiC_Mean_LongDuration']==True)])
        obs_indx = []
        ...
        flareID_postpeak_observations = np.array(param_df['Flare_ID'][(param_df['Cancelled?']==False) & (param_df['HiC_Post_Peak_Obs?']==True)])
        postpeak_obs_indx = []
        for flare in flareID_postpeak_observations:
            indx = np.where(flare==all_flareID)[0][0]
            postpeak_obs_indx.append(indx)
        self.postpeak_obs_array = np.array([False]*len(all_flareID))
        np.put(self.postpeak_obs_array, postpeak_obs_indx, True) 
    ...
    def save_cf_input(self, i):
        ...
        self.score_df.loc[i, 'ObsAfterPeak'] = len(np.where(self.postpeak_obs_array==True)[0])/len(np.where(self.launch_array==True)[0])
