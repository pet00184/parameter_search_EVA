...
class ParameterSearch:
    ...
    def __init__(self, success_flux_key, success_flux_value, parameter_names, parameter_units, parameter_arrays, parameter_combinations, directory):
        ...
        self.launches_df = pd.DataFrame(columns=('Flare_Number', 'Flare_ID', 'Trigger_Time', 'Cancelled?', 'FOXSI_Peak_Observed?', 'HiC_Post_Peak_Obs?', 'Max_FOXSI', 'Mean_FOXSI', 
...
################ Flare Loop Functions ############################################################################   
    ...
    def calculate_observed_xrsb_and_cancellation(self, i):
        ...
        '''
        foxsi_obs_xrsb = np.array(self.data['xrsb'][i][self.foxsi_obs_start:self.foxsi_obs_end])
        hic_obs_xrsb = np.array(self.data['xrsb'][i][self.hic_obs_start:self.hic_obs_end])
        max_mean_list = []
        ...
        if len(foxsi_obs_xrsb) < 6:
            ...
            foxsi_peak_bool = (self.data['time'][i][14] < self.data['time'][i][self.foxsi_obs_start]) &  (self.data['time'][i][self.foxsi_obs_start] < self.data['peak time'][i])
        if len(hic_obs_xrsb) < 6:
            hic_post_peak_bool = math.nan
        else:
            hic_post_peak_bool = self.data['time'][i][self.hic_obs_start] > self.data['peak time'][i]
        ...
        self.calculated_flarelist.append([i, flare_ID, cancellation_bool, trigger_time, foxsi_peak_bool, hic_post_peak_bool, foxsi_max_observed, foxsi_mean_observed, hic_max_observed, hic_mean_observed])
...
################ Post-Loop Functions ############################################################################      
    ...
    def save_flarelist_to_df(self):
        ...
        '''
        self.launches_df[['Flare_Number', 'Flare_ID', 'Cancelled?', 'Trigger_Time', 'FOXSI_Peak_Observed?', 'HiC_Post_Peak_Obs?', 'Max_FOXSI', 'Mean_FOXSI', 'Max_HiC', 'Mean_HiC']] = self.calculated_flarelist  
    ...
    def save_fitsinfo_to_df(self):
        ...
        for f, flare_id in enumerate(self.launches_df['Flare_ID']):
            ...
            self.launches_df.loc[f, 'Start_to_FOXSI_Obs'] = self.data['time'][launched_flare][self.foxsi_obs_start] - self.data['time'][launched_flare][14]
