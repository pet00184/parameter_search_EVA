...
class PlottingResults:
    ...
    def make_full_pr_plot(self, main_key):
        ...
        plt.savefig(os.path.join(self.out_dir, 'Plots', f'fullprplot_{main_key}coded.png'), bbox_inches='tight', dpi=250)
    ...
    def plot_specific_cf(self, param_dict, savestring, big_paramset=False):
        ...
        plt.title(f"Precision = {cf_df.loc[0, 'Precision']:.2f}, Recall = {cf_df.loc[0, 'Recall']:.2f}, \n Launch/Trigger Ratio = {cf_df.loc[0, 'LaunchTriggerRatio']:.2f}, Gordon Score = {cf_df.loc[0, 'Gordon']:.2f}, Peak Ratio = {cf_df.loc[0, 'PeakRatio']:.2f}")
...
class LaunchPlotting:
    ...
    def plot_flare_histogram_includingallflares(self):
        ...
        plt.text(1.01, 0.8, ("Parameters: \n" + "\n".join(param_list)), fontsize=12, transform=ax.transAxes)
        ax.set_title(f'Maximum Flare Flux')
        plt.savefig(os.path.join(out_dir, 'Plots', self.savestring, 'maxflux_allflaresshown_histogram.png'), bbox_inches='tight', dpi=250)
    ...
    def plot_observation_histograms(self, hic=False, cancellation=False):
        ...
        if cancellation:
            ...
            plt.savefig(os.path.join(out_dir, 'Plots', self.savestring, 'meanhic_cancellation_histogram.png'), bbox_inches='tight', dpi=250)
        else:
            ax.set_title(f'Mean Observed Flux for HiC', fontsize=16)
            plt.savefig(os.path.join(out_dir, 'Plots', self.savestring, 'meanhic_launch_histogram.png'), bbox_inches='tight', dpi=250)
    ...
    def duration_histogram(self, cancellation=False):
        ...
        if cancellation:
            ...
            plt.savefig(os.path.join(out_dir, 'Plots', self.savestring, 'duration_cancellations.png'), bbox_inches='tight', dpi=250)
        else:
            ax.set_title('Duration of Launches')
            plt.savefig(os.path.join(out_dir, 'Plots', self.savestring, 'duration_launches.png'), bbox_inches='tight', dpi=250)
    ...
    def plot_longdurationobs_hist(self):
        df = self.launch_df
        param_units = [df.loc[0, f'{key}_units'] for key in self.combo_dict.keys()]
        param_list = [f'{nice_key}={val} {unit}' for (key, val), unit, nice_key in zip(self.combo_dict.items(), param_units, self.nice_keys_list)]
        
        long_dur_hic = df[df['HiC_Mean_LongDuration']==True]
        above_c5 = long_dur_hic[long_dur_hic['Flare_C5']==True]
        below_c5 = long_dur_hic[long_dur_hic['Flare_C5']==False]
        ...
        ax.legend()
        ax.set_title('Flare Class for Long Duration HiC Observations')
        plt.savefig(os.path.join(out_dir, 'Plots', self.savestring, f'hiclongdurobs_flareclass.png'), bbox_inches='tight', dpi=250)    
