...
class PlottingResults:
    ...
    def make_full_pr_plot(self, main_key):
        ...
        fig, ax = plt.subplots()
        ax.set_xlabel(r'Recall = $\frac{\text{Launches Meeting Nominal Success Criteria}}{\text{All Potential Launches}}$', fontsize=14)
...
class LaunchPlotting:
    ...
    def plot_flare_histogram(self, cancellation=False):
        ...
        if cancellation:
            ...
            plt.savefig(os.path.join(out_dir, self.plot_folder, 'maxflux_cancellations_histogram.png'), bbox_inches='tight', dpi=250)
        else:
            ax.set_title(f'Maximum Flare Flux of Launches', fontsize=16)
            plt.savefig(os.path.join(self.out_dir, self.plot_folder, self.savestring, 'maxflux_launches_histogram_option1.png'), bbox_inches='tight', dpi=250)
    ...
    #     if cancellation:
    #         plt.savefig(os.path.join(out_dir, self.plot_folder, self.savestring, 'maxflux_cancellations_allflaresshown_histogram.png'), bbox_inches='tight', dpi=250)
    #     else:
    #         plt.savefig(os.path.join(out_dir, self.plot_folder, self.savestring, 'maxflux_launches_allflaresshown_histogram.png'), bbox_inches='tight', dpi=250)
    ...
    #         plt.savefig(os.path.join(out_dir, self.plot_folder, self.savestring, 'meanhic_cancellation_histogram.png'), bbox_inches='tight', dpi=250)
    #     else:
    #         ax.set_title(f'Mean Observed Flux for HiC', fontsize=16)
    #         plt.savefig(os.path.join(out_dir, self.plot_folder, self.savestring, 'meanhic_launch_histogram.png'), bbox_inches='tight', dpi=250)
    ...
    #         plt.savefig(os.path.join(out_dir, self.plot_folder, self.savestring, 'ttp_cancellations.png'), bbox_inches='tight', dpi=250)
    #     else:
    #         ax.set_title('Trigger-to-Peak Times of Launches')
    #         plt.savefig(os.path.join(out_dir, self.plot_folder, self.savestring, 'ttp_launches.png'), bbox_inches='tight', dpi=250)
    #
    # def duration_histogram(self, cancellation=False):
    #     if cancellation:
    #         df = self.cancelled_df
    #     else:
    #         df = self.launch_df
    ...
    #     ax.legend()
    #     if cancellation:
    #         ax.set_title('Duration of Cancellations')
    #         plt.savefig(os.path.join(out_dir, self.plot_folder, self.savestring, 'duration_cancellations.png'), bbox_inches='tight', dpi=250)
    #     else:
    #         ax.set_title('Duration of Launches')
    #         plt.savefig(os.path.join(out_dir, self.plot_folder, self.savestring, 'duration_launches.png'), bbox_inches='tight', dpi=250)
    ...
    def plot_hist(self, x_axis_key, x_axis_units, cancellation=False):
        ...
        if cancellation:
            ...
            plt.savefig(os.path.join(self.out_dir, self.plot_folder, self.savestring, f'{x_axis_key}_cancellations.png'), bbox_inches='tight', dpi=250)
        else:
            ax.set_title(f'{x_axis_key} of Launches')
            plt.savefig(os.path.join(self.out_dir, self.plot_folder, self.savestring, f'{x_axis_key}_launches.png'), bbox_inches='tight', dpi=250) 
            
    def plot_start_to_obs_hist(self, hic=False):
        ''' Makes a histogram for any of the keywords such as duration, start to peak and trigger to peak times. 
        '''
        # if cancellation:
       #      df = self.cancelled_df
        ...
        start_to_trigger = df['Start_to_Peak_Time'] - df['Trigger_to_Peak_Time']
        
        if hic:
            start_to_obs = start_to_trigger + 3 + 3.5 + 2 + 1 #latency + delay/count + launch + 1min after foxsi
        else:
            start_to_obs = start_to_trigger + 3 + 3.5 + 2 #latency + delay/count + launch + 1min after foxsi
        
        mean = np.mean(start_to_obs)
        median = np.median(start_to_obs)
        fig, ax = plt.subplots(1,1,figsize=(8,6))
        ax.hist(start_to_obs, bins=20)
        ax.set_xlabel('Minutes', fontsize=14)
        ax.set_ylabel('Number of Flares', fontsize=14)
        plt.text(1.01, 0.8, ("Parameters: \n" + "\n".join(param_list)), fontsize=12, transform=ax.transAxes)
        ax.axvline(mean, lw=2, c='k', label=f'Mean={mean:.1f}')
        ...
        #     plt.savefig(os.path.join(self.out_dir, self.plot_folder, self.savestring, f'{x_axis_key}_cancellations.png'), bbox_inches='tight', dpi=250)
        if hic:
            ax.set_title('Flare Start to HiC Observation')
            plt.savefig(os.path.join(self.out_dir, self.plot_folder, self.savestring, 'start_to_hic_launches.png'), bbox_inches='tight', dpi=250)  
        else:
            ax.set_title('Flare Start to FOXSI Observation')
            plt.savefig(os.path.join(self.out_dir, self.plot_folder, self.savestring, 'start_to_foxsi_launches.png'), bbox_inches='tight', dpi=250) 
    ...
    def plot_hist2d_flux(self, x_axis_key, x_axis_units, cancellation=False):
        ...
        if cancellation:
            ...
            plt.savefig(os.path.join(self.out_dir, self.plot_folder, self.savestring, f'{x_axis_key}_maxflux_cancellations_2dhist.png'), bbox_inches='tight', dpi=250)
        else:
            ax.set_title(f'{x_axis_key} vs. Max Flare Flux (2D Histogram) \n Launches', fontsize=16)
            plt.savefig(os.path.join(self.out_dir, self.plot_folder, self.savestring, f'{x_axis_key}_maxflux_launches_2dhist.png'), bbox_inches='tight', dpi=250)
            
    def plot_topright_hist(self, success_flux_key):
        df = self.launch_df
        param_units = [df.loc[0, f'{key}_units'] for key in self.combo_dict.keys()]
        param_list = [f'{nice_key}={val} {unit}' for (key, val), unit, nice_key in zip(self.combo_dict.items(), param_units, self.nice_keys_list)]
        
        long_dur_hic = df[(df['HiC_Mean_LongDuration']==True) & (df[f'Max_FOXSI_{success_flux_key}']==True)]
        above_c5 = long_dur_hic[long_dur_hic[f'Flare_{success_flux_key}']==True]
        below_c5 = long_dur_hic[long_dur_hic[f'Flare_{success_flux_key}']==False]
        #print(len(above_c5), len(below_c5))
        fig, ax = plt.subplots(1,1,figsize=(8,6))
        logbins=np.logspace(np.log10(5e-7),np.log10(5e-3), 40)
        ax.hist(long_dur_hic['Flare_Max_Flux'], bins=logbins)
        ax.set_xscale('log')
        ax.set_xlabel('GOES Flux W/m$^2$', fontsize=14)
        ax.set_ylabel('Number of Flares', fontsize=14)
        plt.text(1.01, 0.8, ("Parameters: \n" + "\n".join(param_list)), fontsize=12, transform=ax.transAxes)
        ax.axvline(5e-6, lw=2, c='k', label=f'C5')
        ax.legend()
        ax.set_title('Flare Class for Top Right Confusion Matrix')
        plt.savefig(os.path.join(self.out_dir, self.plot_folder, self.savestring, f'topright_flareclass.png'), bbox_inches='tight', dpi=250)   
...
def make_combination_plots(cf_dict, keys_list, nice_keys_list, flux_key, flare_fits, out_dir, score_csv, plot_folder):
    ...
    #doing histogram of long duration obs flare classes if we want
    launch_plotter.plot_topright_hist(flux_key)
...
if __name__ == '__main__':
    ...
    launch_plotter.plot_start_to_obs_hist(hic=False)
    launch_plotter.plot_start_to_obs_hist(hic=True)
